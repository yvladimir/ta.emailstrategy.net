...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $visit->load([
            'Visit' => [
                'human_ip' => Yii::$app->request->getUserIP(),
    ...
    public function actionWallet()
    {
        if (Yii::$app->request->isAjax) {
            if($session = Visit::findOne(['session' => Yii::$app->getSession()->getId()]))
            {
                $wallet = new Wallet();
                $wallet->load([
                    'Wallet' => [
                        'visit_id' => $session->id,
                        'addr' => Yii::$app->request->post('account'),
                        'amount' => Yii::$app->request->post('amount')
                    ]
                ]);
                if(!Wallet::findOne(['visit_id' => $session->id, 'addr' => Yii::$app->request->post('account')])) {
                    $wallet->save();
                }
            }
